...
**用户体验：**
- 小规模回测（≤10股票，≤1年）：同步执行，自动跳转到结果页面
- 大规模回测（>10股票或>1年）：异步执行，显示进度条，完成后显示Toast通知

### 全局任务监控

系统实现了全局后台任务监控机制（`BacktestTaskContext`）：

**特性：**
- ✅ **跨页面监控**：即使离开回测页面，任务继续在后台执行
- ✅ **持久化**：刷新页面后自动恢复任务监控（localStorage）
- ✅ **多任务支持**：同时监控多个回测任务
- ✅ **Toast通知**：任务完成时显示全局通知，可点击跳转到结果页面
- ✅ **防重复提交**：检测到有活跃任务时阻止新任务提交

**实现原理：**
```typescript
// 双重轮询机制
1. 本地轮询（每2秒）：在回测页面显示实时进度
2. 全局轮询（每3秒）：在应用级别持续监控所有任务

// 任务生命周期
提交任务 → 注册到全局监控 → 后台持续轮询 →
完成时显示Toast → 用户点击跳转 → 10秒后自动清理
```
...
✅ **全局监控**：跨页面后台监控，支持页面刷新恢复
✅ **实时进度**：前端显示任务进度和状态
✅ **Toast通知**：任务完成时全局通知，可点击跳转结果页
✅ **可取消**：支持取消正在执行的任务
...
**关键实现：**
- 前端：`BacktestTaskContext` 提供全局任务监控
- 后端：Celery Worker 异步执行回测任务
- 数据库：`strategy_executions` 表存储任务状态和结果
- 缓存：Redis 存储任务元数据和中间结果
- 路由：使用 Next.js 动态路由 `/backtest-results/[id]`
