...
2. ✅ MeanReversionStrategy（均值回归策略）
3. ✅ MLStrategy（机器学习策略）
4. ✅ MultiFactorStrategy（多因子策略）
5. ✅ StrategyCombiner（策略组合器）
...
| 问题编号 | 问题描述 | 严重程度 | 影响范围 |
|---------|---------|---------|---------|
| **P1** | Core 有 5 个策略，前端只显示 1 个（实际2个） | 🔴 高 | 功能完整性 |
| **P2** | 缺少策略列表页面，用户无法浏览所有策略 | 🔴 高 | 用户体验 |
| **P3** | 历史记录未持久化，刷新页面数据丢失 | 🟠 中 | 用户体验 |
| **P4** | 缺少策略详情页面，用户不了解策略原理 | 🟠 中 | 可用性 |
| **P5** | AI 模型与策略的关系不清晰 | 🟡 低 | 概念理解 |
| **P6** | 配置面板过于复杂，缺少向导模式 | 🟡 低 | 易用性 |

### 2.2 数据差异分析

| 层级 | 策略数量 | 策略列表 |
|------|---------|---------|
| Core 模块 | 5个 | Momentum, MeanReversion, ML, MultiFactor, Combiner |
| Backend 模块 | 2个 | ComplexIndicator, MLModel |
...
1. **创建新的基类文件**
   ```bash
   # 在 backend/app/strategies/ 下创建新架构文件
   touch backend/app/strategies/stock_selector.py
   touch backend/app/strategies/entry_strategy.py
   touch backend/app/strategies/exit_strategy.py
   touch backend/app/strategies/strategy_composer.py
   ```

2. **实现 StockSelector 基类**
   ```python
...
| 任务 | 工作量 | 优先级 | 产出 |
|------|--------|--------|------|
| 0.1 设计三层基类 | 3-5天 | P0 | 4个基类文件 |
| 0.2 基础选股器 | 3-4天 | P0 | 3个选股器实现 |
| 0.3 基础入场策略 | 3-4天 | P0 | 3个入场策略 |
| 0.4 基础退出策略 | 3-4天 | P0 | 4个退出策略 |
| 0.5 回测引擎适配 | 3-5天 | P0 | 新回测方法 |
| 0.6 API 端点 | 2-3天 | P0 | 5个新端点 |
| **合计** | **17-25天** | | **完整三层架构** |
...
#### 任务 1.1：迁移 Core 策略到 Backend

**目标：** 将 Core 中的 3 个策略移植到 Backend 生产环境

| 策略名称 | 源文件 | 目标文件 | 工作量 |
|---------|--------|---------|--------|
| MomentumStrategy | `core/src/strategies/momentum_strategy.py` | `backend/app/strategies/momentum_strategy.py` | 1-2天 |
| MeanReversionStrategy | `core/src/strategies/mean_reversion_strategy.py` | `backend/app/strategies/mean_reversion_strategy.py` | 1-2天 |
| MultiFactorStrategy | `core/src/strategies/multi_factor_strategy.py` | `backend/app/strategies/multi_factor_strategy.py` | 1-2天 |

**实施步骤：**

1. **创建策略文件**
   ```bash
   # 在 backend/app/strategies/ 下创建新文件
   touch backend/app/strategies/momentum_strategy.py
   touch backend/app/strategies/mean_reversion_strategy.py
   touch backend/app/strategies/multi_factor_strategy.py
   ```
...
| 端点 | 方法 | 功能 | 状态 |
|------|------|------|------|
| `/api/strategy/list` | GET | 获取策略列表 | ✅ 已有 |
| `/api/strategy/metadata` | GET | 获取策略元数据 | ✅ 已有 |
| `/api/backtest/run` | POST | 运行回测 | ✅ 已有 |
...
| 路由 | 页面 | 功能 | 状态 |
|------|------|------|------|
| `/strategies` | 策略列表页 | 浏览所有策略 | ⭕ 新增 |
| `/strategies/[id]` | 策略详情页 | 查看策略详情 | ⭕ 新增 |
| `/backtest` | 回测执行页 | 配置并运行回测 | ✅ 已有 |
| `/my-backtests` | 历史记录页 | 查看回测历史 | ⭕ 新增 |
| `/my-backtests/[id]` | 回测详情页 | 查看单条记录详情 | ⭕ 新增 |
...
| | 创建 API 端点 | 2-3天 | P0 |
| **小计** | | **17-25天** | **最高优先级** |
| **阶段一** | 迁移 MomentumStrategy | 1-2天 | P1 |
| | 迁移 MeanReversionStrategy | 1-2天 | P1 |
| | 迁移 MultiFactorStrategy | 1-2天 | P1 |
| | 注册策略 + 测试 | 0.5天 | P1 |
| **小计** | | **3.5-6.5天** | |
| **阶段二** | 创建策略列表页面 | 1-2天 | P1 |
| | 创建策略详情页面 | 1-2天 | P1 |
| | 更新导航栏 | 0.5天 | P1 |
| | 集成测试 | 0.5天 | P1 |
| **小计** | | **3-5天** | |
| **阶段三** | 后端 API 开发 | 1-2天 | P2 |
| | 数据库迁移 | 0.5天 | P2 |
| | 前端历史记录页面 | 2-3天 | P2 |
| | 前端详情页面 | 1天 | P2 |
| | 更新回测页面保存逻辑 | 0.5天 | P2 |
| **小计** | | **5-7天** | |
| **阶段四** | AI 生成服务开发（Prompt + API集成） | 2-3天 | P1 🚀 |
| | 代码验证器 + 沙箱 | 2-3天 | P1 |
| | 数据库表设计和迁移 | 0.5天 | P1 |
| | API 端点开发 | 1-2天 | P1 |
| | 前端 AI 生成器 UI | 2-3天 | P1 |
| | 集成测试和 Prompt 调优 | 1-2天 | P1 |
| **小计** | | **8.5-13.5天** | |
| **阶段五** | 完善 StrategyCombiner | 1-2天 | P3 |
| | 新增 SignalFilter | 1-2天 | P3 |
| | 测试和优化 | 1天 | P3 |
| **小计** | | **3-5天** | |
| **总计** | | **40.5-62天** | **约 8-12周** |
...
**重要说明：** 由于采用三层架构重构，总工期从原计划的 5-6周 延长至 10-12周，但带来的价值巨大：
...
**第1-4周：三层架构重构（P0 ⭐ 最高优先级）**

**Week 1: 基础架构设计**
- Day 1-2: 设计并实现三层基类（StockSelector, EntryStrategy, ExitStrategy, StrategyComposer）
- Day 3-4: 实现 MomentumSelector 和 ValueSelector
- Day 5: 实现 ExternalSelector（StarRanker 集成）
- Day 6-7: 单元测试和代码审查

**Week 2: 入场策略实现**
- Day 1-2: 实现 MABreakoutEntry（均线突破）
- Day 3-4: 实现 RSIOversoldEntry（RSI超卖）
- Day 5: 实现 ImmediateEntry（立即入场）
- Day 6-7: 单元测试和参数调优

**Week 3: 退出策略实现**
- Day 1-2: 实现 ATRStopLossExit（动态止损）
- Day 3: 实现 FixedStopLossExit（固定止损）
- Day 4: 实现 TimeBasedExit（时间止损）
- Day 5: 实现 CombinedExit（组合退出）
- Day 6-7: 单元测试和集成测试

**Week 4: 回测引擎与API**
- Day 1-3: 修改 BacktestEngine 支持三层架构
- Day 4-5: 创建 REST API 端点
- Day 6-7: 端到端集成测试 + 性能测试

**第5周：补全策略库（P1）**
- Day 1-2: 迁移 MomentumStrategy
- Day 3-4: 迁移 MeanReversionStrategy
- Day 5-6: 迁移 MultiFactorStrategy
- Day 7: 测试和修复
...
这是一次值得投入的架构升级，将使本项目在同类产品中具有显著竞争优势。
