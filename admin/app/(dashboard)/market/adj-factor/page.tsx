...
export default function AdjFactorPage() {
  ...
  // 表格列定义
  const columns: Column<AdjFactorData>[] = useMemo(() => [
    {
      key: 'trade_date',
      header: '交易日期',
    ...
    },
    {
      key: 'adj_factor',
      header: '复权因子',
      accessor: (row) => row.adj_factor !== null ? row.adj_factor.toFixed(6) : '-'
    }
  ], [])

  // 移动端卡片视图
  const mobileCard = useCallback((item: AdjFactorData) => (
    <div className="space-y-2">
      ...
      <div className="flex justify-between items-center">
        <span className="text-sm text-gray-600 dark:text-gray-400">复权因子</span>
        <span className="font-medium">
          {item.adj_factor !== null ? item.adj_factor.toFixed(6) : '-'}
        </span>
      </div>
    </div>
  ), [])
    ...
    <div className="space-y-6">
      ...
      {statistics && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">平均复权因子</CardTitle>
              <TrendingUp className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {statistics.avg_factor !== null ? statistics.avg_factor.toFixed(2) : '-'}
              </div>
            </CardContent>
          </Card>
