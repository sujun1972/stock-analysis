...
class ApiClient {
   ...
   * 获取板块资金流向数据
   */
  async getMoneyflowIndDc(params: {
    start_date?: string  // YYYY-MM-DD
    end_date?: string    // YYYY-MM-DD
    ...
    const response = await axiosInstance.get('/api/moneyflow-ind-dc', { params })
    return response.data
  }
   ...
   * 通过Celery任务异步执行，立即返回任务ID
   */
  async syncMoneyflowIndDcAsync(params?: {
    trade_date?: string  // YYYY-MM-DD
    start_date?: string  // YYYY-MM-DD
    end_date?: string    // YYYY-MM-DD
    ...
    const response = await axiosInstance.post('/api/moneyflow-ind-dc/sync-async', null, { params })
    return response.data
  }
   ...
   * 获取最新板块资金流向数据
   */
  async getLatestMoneyflowIndDc(params?: {
    content_type?: string  // 数据类型(行业、概念、地域)
    limit?: number
  }): Promise<ApiResponse<any>> {
    const response = await axiosInstance.get('/api/moneyflow-ind-dc/latest', { params })
    return response.data
  }
   ...
   * 重置用户配额
   */
  async resetUserQuota(userId: number): Promise<ApiResponse<any>> {
    const response = await axiosInstance.post(`/api/users/${userId}/reset-quota`)
    return response.data
  }
   ...
   * 更新用户配额
   */
  async updateUserQuota(userId: number, data: {
    backtest_quota_total?: number
    ml_prediction_quota_total?: number
    max_strategies?: number
  }): Promise<ApiResponse<any>> {
    const response = await axiosInstance.patch(`/api/users/${userId}/quota`, data)
    return response.data
  }
   ...
   * 获取用户登录历史
   */
  async getUserLoginHistory(userId: number, limit: number = 50): Promise<ApiResponse<any[]>> {
   ...
   * 获取用户操作日志
   */
  async getUserActivityLogs(userId: number, limit: number = 50, action_type?: string): Promise<ApiResponse<any[]>> {
  ...
  async syncSentimentData(date?: string): Promise<ApiResponse<any>> {
    const response = await axiosInstance.post('/api/sentiment/sync', null, {
      params: { date }
    })
    return response.data
  }
  ...
  async getTradingCalendar(params?: {
    ...
    const response = await axiosInstance.get('/api/sentiment/calendar', { params })
    return response.data
  }
