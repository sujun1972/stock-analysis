    ...
    try:
        ...
        for row in results:
            ...
            model_data = {
                ...
                "feature_importance": (
                    sanitize_float_values(feature_importance) if feature_importance else None
                ),
...
async def get_feature_snapshot(symbol: str, date: str, model_id: Optional[str] = None):
    ...
    try:
        if not model_id:
            raise HTTPException(
                status_code=400, detail="请先选择一个模型，特征快照需要从模型的训练数据中提取"
            )
        ...
        if not os.path.exists(features_file):
            # 如果没有保存的特征文件，返回提示信息
            raise HTTPException(
                status_code=404, detail="该模型没有保存特征数据，请重新训练模型以保存特征快照"
            )
        ...
        if X is None or len(X) == 0:
            raise HTTPException(status_code=404, detail="特征数据为空")
        ...
        # 获取目标值
        target = (
            float(y.loc[date_obj]) if date_obj in y.index and pd.notna(y.loc[date_obj]) else None
        )
