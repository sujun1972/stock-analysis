...
async def sync_stk_high_shock_full_history(
    ...
    """全量同步个股严重异常波动历史数据（按月切片，支持 Redis 续继，策略从 sync_configs 读取）"""
    try:
        from app.tasks.stk_high_shock_tasks import sync_stk_high_shock_full_history_task
        from app.repositories.sync_config_repository import SyncConfigRepository
        from app.api.endpoints.sync_dashboard import release_stale_lock
        await asyncio.to_thread(release_stale_lock, 'stk_high_shock')

        start_date_formatted = start_date.replace('-', '') if start_date else None

        sync_config_repo = SyncConfigRepository()
        cfg = await asyncio.to_thread(sync_config_repo.get_by_table_key, 'stk_high_shock')
...
async def sync_stk_high_shock_async(
    ...
    try:
        ...
        cfg = await asyncio.to_thread(SyncConfigRepository().get_by_table_key, 'stk_high_shock')
        ...
        if not start_date_formatted and sync_strategy in ('by_date_range', 'by_month', 'by_week', 'by_date', 'by_ts_code'):
            from app.services.stk_high_shock_service import StkHighShockService
            suggested = await StkHighShockService().get_suggested_start_date()
            if suggested:
                start_date_formatted = suggested
                logger.info(f"个股严重异常波动增量同步：未传 start_date，自动使用建议起始日期 {start_date_formatted}")
        ...
        logger.info(f"个股严重异常波动增量同步任务已提交: {celery_task.id} strategy={sync_strategy}")
