...
async def get_cashflow_data(
    ...
    try:
        ...
        result = await service.get_cashflow_data(
...
async def sync_cashflow_async(
    ...
    """
    try:
        from app.tasks.cashflow_tasks import sync_cashflow_task
        ...
        # 提交 Celery 任务
        celery_task = sync_cashflow_task.apply_async(
            kwargs={
                'ts_code': ts_code,
                'period': period_fmt,
        ...
        task_data = await helper.create_task_record(
            ...
            task_name='tasks.sync_cashflow',
            display_name='现金流量表数据同步',
            task_type='data_sync',
        ...
        return ApiResponse.success(
            data=task_data,
            message="现金流量表同步任务已提交，请在任务面板查看进度"
        )
...
async def sync_cashflow_full_history_async(
    ...
    try:
        ...
        if concurrency is None:
            ...
            cfg = await asyncio.to_thread(sync_config_repo.get_by_table_key, 'cashflow')
