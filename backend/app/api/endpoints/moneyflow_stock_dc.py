...
提供个股主力资金流向数据接口，包含超大单、大单、中单、小单的流入流出情况，
...
async def get_top_moneyflow_stocks(
    ...
    """
    获取主力资金流入排名前N的股票（默认最新交易日）
    """
    try:
        # 如果没有指定日期，获取最新交易日
        if not trade_date:
            date_query = text("SELECT MAX(trade_date) FROM moneyflow_stock_dc")
