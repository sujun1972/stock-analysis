...
async def get_ggt_top10(
    ...
    try:
        ...
        result = await service.get_ggt_top10_data(
...
async def sync_ggt_top10_async(
    ts_code: Optional[str] = Query(None, description="股票代码"),
    ...
    """
    try:
        from app.tasks.ggt_top10_tasks import sync_ggt_top10_task
        ...
        # 提交Celery任务（异步执行）
        celery_task = sync_ggt_top10_task.apply_async(
            kwargs={
                'ts_code': ts_code,
        ...
        task_data = await helper.create_task_record(
            ...
            task_name='tasks.sync_ggt_top10',
        ...
        logger.info(f"港股通十大成交股同步任务已提交: {celery_task.id}")
