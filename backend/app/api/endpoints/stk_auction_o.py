...
async def get_stk_auction_o(
    ...
    Returns:
        开盘集合竞价数据列表和统计信息，含 trade_date 用于前端回填
    """
    try:
...
async def sync_async(
    ...
    try:
        ...
        cfg = await asyncio.to_thread(SyncConfigRepository().get_by_table_key, 'stk_auction_o')
        ...
        if not start_date_formatted and sync_strategy in ('by_date_range', 'by_month', 'by_week', 'by_date', 'by_ts_code'):
            suggested = await StkAuctionOService().get_suggested_start_date()
            if suggested:
                start_date_formatted = suggested
                logger.info(f"开盘集合竞价增量同步：未传 start_date，自动使用建议起始日期 {start_date_formatted}")
        ...
        logger.info(f"股票开盘集合竞价数据同步任务已提交: {celery_task.id} strategy={sync_strategy}")
...
async def sync_stk_auction_o_full_history(
    ...
    try:
        from app.tasks.stk_auction_o_tasks import sync_stk_auction_o_full_history_task
        from app.repositories.sync_config_repository import SyncConfigRepository
        from app.api.endpoints.sync_dashboard import release_stale_lock

        await asyncio.to_thread(release_stale_lock, 'stk_auction_o')
        ...
        celery_task = sync_stk_auction_o_full_history_task.apply_async(
            kwargs={
                'start_date': start_date_formatted,
        ...
        task_data = await helper.create_task_record(
            celery_task_id=celery_task.id,
            task_name='tasks.sync_stk_auction_o_full_history',
            display_name='股票开盘集合竞价全量同步',
            task_type='data_sync',
            user_id=current_user.id,
            task_params={
                'start_date': start_date_formatted,
        ...
        logger.info(f"股票开盘集合竞价全量同步任务已提交: {celery_task.id}")
        return ApiResponse.success(data=task_data, message="全量同步任务已提交")

    except Exception as e:
        logger.error(f"提交股票开盘集合竞价全量同步任务失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))
