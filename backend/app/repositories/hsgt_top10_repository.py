"""
沪深股通十大成交股 Repository

管理 hsgt_top10 表的数据访问
"""
...
class HsgtTop10Repository(BaseRepository):
    ...
    def __init__(self, db=None):
        super().__init__(db)
        logger.debug("✓ HsgtTop10Repository initialized")
    ...
    def bulk_upsert(self, df: pd.DataFrame) -> int:
        ...
        for _, row in df.iterrows():
            values.append((
                ...
                to_python_type(row.get('buy')),
                to_python_type(row.get('sell'))
            ))
    ...
    def _row_to_dict(self, row: tuple) -> Dict:
        ...
        return {
            ...
            'change': float(row[4]) if row[4] is not None else None,
            'rank': row[5],
            'market_type': row[6],
            'amount': float(row[7]) if row[7] is not None else None,
            'net_amount': float(row[8]) if row[8] is not None else None,
            'buy': float(row[9]) if row[9] is not None else None,
            'sell': float(row[10]) if row[10] is not None else None
        }
