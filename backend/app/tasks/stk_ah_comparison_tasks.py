...
@celery_app.task(bind=True, name="tasks.sync_stk_ah_comparison")
def sync_stk_ah_comparison_task(
    self,
    ...
    Args:
        hk_code: 港股代码 (xxxxx.HK)
        ts_code: A股代码 (xxxxxx.SH/SZ/BJ)
    ...
    """
    try:
        logger.info(f"开始执行AH股比价同步任务: hk_code={hk_code}, ts_code={ts_code}, trade_date={trade_date}, start_date={start_date}, end_date={end_date}")
        ...
        if result["status"] == "success":
            ...
            logger.warning(f"AH股比价同步失败: {result}")
            error_msg = result.get('error', '未知错误')
            raise Exception(f"同步失败: {error_msg}")

    except Exception as e:
        logger.error(f"执行AH股比价同步任务失败: {str(e)}")
        import traceback
...
def sync_stk_ah_comparison_full_history_task(
    self,
    start_date: Optional[str] = None,
    concurrency: int = 5,
    ...
    LOCK_KEY = 'sync:stk_ah_comparison:full_history:lock'
