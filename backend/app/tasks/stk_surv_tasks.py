...
@celery_app.task(bind=True, name="tasks.sync_stk_surv")
def sync_stk_surv_task(
    self,
    ...
    """
    try:
        logger.info(f"开始执行机构调研数据同步任务: ts_code={ts_code}, trade_date={trade_date}, start_date={start_date}, end_date={end_date}")
        ...
        if result["status"] == "success":
            logger.info(f"机构调研数据同步成功: {result['records']} 条")
            return result
        else:
            logger.warning(f"机构调研数据同步失败: {result}")
            error_msg = result.get('error', '未知错误')
            raise Exception(f"同步失败: {error_msg}")

    except Exception as e:
        logger.error(f"执行机构调研数据同步任务失败: {str(e)}")
        import traceback
...
def sync_stk_surv_full_history_task(
    self,
    start_date: Optional[str] = None,
    concurrency: int = 5,
    ...
    LOCK_KEY = 'sync:stk_surv:full_history:lock'
