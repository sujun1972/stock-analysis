...
def sync_stk_high_shock_task(
    ...
    try:
        logger.info(
            f"开始执行个股严重异常波动增量同步任务: strategy={sync_strategy} "
...
def sync_stk_high_shock_full_history_task(
    self,
    start_date: Optional[str] = None,
    concurrency: int = 5,
    ...
    logger.info(f"========== [Celery] 开始全量历史个股严重异常波动同步任务 strategy={strategy} concurrency={concurrency} ==========")

    if redis_client is None:
        return {"status": "error", "message": "Redis 不可用"}

    with redis_lock.acquire(StkHighShockService.FULL_HISTORY_LOCK_KEY, timeout=10800, blocking=False) if redis_lock else _DummyContext() as acquired:
