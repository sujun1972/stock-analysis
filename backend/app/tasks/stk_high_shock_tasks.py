...
def sync_stk_high_shock_full_history_task(self, start_date: str = None):
    """全量同步个股严重异常波动数据（单次请求）

    stk_high_shock 接口为快照接口，不支持日期范围过滤，单次拉取全量当前数据。
    start_date 参数保留以与其他全量同步任务保持接口一致，不会传给接口。
    """
    from app.core.redis_lock import redis_client

    logger.info("========== [Celery] 开始全量历史个股严重异常波动同步任务 ==========")

    if redis_client is None:
        logger.error("Redis 不可用，无法执行全量同步任务")
        return {"status": "error", "message": "Redis 不可用"}

    with redis_lock.acquire(StkHighShockService.FULL_HISTORY_LOCK_KEY, timeout=7200, blocking=False) if redis_lock else _DummyContext() as acquired:
        if not acquired and redis_lock:
            logger.warning("⚠️  全量个股严重异常波动同步任务已在执行中，跳过本次执行")
    ...
    logger.info(f"========== [Celery] 全量历史个股严重异常波动同步结束: {result} ==========")
    return result
