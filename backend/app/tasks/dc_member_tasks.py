...
@celery_app.task(bind=True, name="tasks.sync_dc_member")
def sync_dc_member_task(
    self,
    ...
    """
    try:
        logger.info(f"开始执行东方财富板块成分同步任务: ts_code={ts_code}, con_code={con_code}, "
        ...
        if result["status"] == "success":
            ...
            logger.warning(f"东方财富板块成分同步失败: {result}")
            error_msg = result.get('error', '未知错误')
            raise Exception(f"同步失败: {error_msg}")

    except Exception as e:
        logger.error(f"执行东方财富板块成分同步任务失败: {str(e)}")
        import traceback
...
def sync_dc_member_full_history_task(
    self,
    start_date: Optional[str] = None,
    concurrency: int = 5,
    **kwargs
):
    """按自然月切片全量同步东方财富板块成分历史数据（支持中断续继）"""
    from app.core.redis_lock import redis_client, redis_lock
    from app.tasks.sync_tasks import _DummyContext

    LOCK_KEY = "sync:dc_member:full_history:lock"
    logger.info(f"[Celery] 开始东方财富板块成分全量历史同步 start_date={start_date} concurrency={concurrency}")
    ...
    logger.info(f"[Celery] 东方财富板块成分全量历史同步结束: 成功={result.get('success')}, 跳过={result.get('skipped')}, 失败={result.get('errors')}")
    return result
