...
@celery_app.task(bind=True, name="tasks.sync_ccass_hold")
def sync_ccass_hold_task(
    self,
    ...
    """
    try:
        logger.info(f"开始执行中央结算系统持股汇总数据同步任务: ts_code={ts_code}, hk_code={hk_code}, "
        ...
        if result["status"] == "success":
            logger.info(f"中央结算系统持股汇总数据同步成功: {result['records']} 条")
            return result
        else:
            logger.warning(f"中央结算系统持股汇总数据同步失败: {result}")
            error_msg = result.get('error', '未知错误')
            raise Exception(f"同步失败: {error_msg}")

    except Exception as e:
        logger.error(f"执行中央结算系统持股汇总数据同步任务失败: {str(e)}")
        import traceback
...
def sync_ccass_hold_full_history_task(
    self,
    start_date: Optional[str] = None,
    concurrency: int = 5,
    ...
    LOCK_KEY = 'sync:ccass_hold:full_history:lock'
