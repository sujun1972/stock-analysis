...
EXPRESS_FULL_HISTORY_LOCK_KEY = "sync:express:full_history:lock"
...
def sync_express_full_history_task(self, start_date: str = None, concurrency: int = 5):
    """按季度报告期全量同步业绩快报历史数据（支持中断续继）"""
    from app.core.redis_lock import redis_client

    logger.info(f"========== [Celery] 开始全量业绩快报同步任务，start_date={start_date}, concurrency={concurrency} ==========")

    if redis_client is None:
        logger.error("Redis 不可用，无法执行全量同步任务")
        return {"status": "error", "message": "Redis 不可用"}

    with redis_lock.acquire(EXPRESS_FULL_HISTORY_LOCK_KEY, timeout=28800, blocking=False) if redis_lock else _DummyContext() as acquired:
        if not acquired and redis_lock:
            logger.warning("⚠️  全量业绩快报同步任务已在执行中，跳过本次执行")
    ...
    logger.info(f"========== [Celery] 全量业绩快报同步结束: {result} ==========")
    return result
