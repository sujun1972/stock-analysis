...
DAILY_BASIC_FULL_HISTORY_PROGRESS_KEY = "sync:daily_basic:full_history:progress"


@celery_app.task(bind=True, name="tasks.sync_daily_basic")
def sync_daily_basic_task(
    self,
    ...
    """
    try:
        logger.info(f"开始执行每日指标同步任务: trade_date={trade_date}, ts_code={ts_code}, start_date={start_date}, end_date={end_date}")
        ...
        if result["status"] == "success":
            logger.info(f"每日指标同步成功: {result['records']} 条")
            return result
        else:
            logger.warning(f"每日指标同步失败: {result}")
            error_msg = result.get('error', '未知错误')
            raise Exception(f"同步失败: {error_msg}")

    except Exception as e:
        logger.error(f"执行每日指标同步任务失败: {str(e)}")
        import traceback
...
def sync_daily_basic_full_history_task(self, start_date: Optional[str] = None, concurrency: int = 8):
    """
    逐只股票全量同步每日指标历史数据

    每只股票单独请求 Tushare，8 并发执行，支持 Redis 中断续继。
    进度存储：Redis Set key = sync:daily_basic:full_history:progress
    ...
    logger.info(f"========== [Celery] 开始每日指标全量历史同步任务 start_date={effective_start} ==========")
    ...
    # 读取已完成的进度
    completed_set = redis_client.smembers(DAILY_BASIC_FULL_HISTORY_PROGRESS_KEY)
    ...
    async def run_concurrent():
        ...
        for batch_start in range(0, len(pending_codes), BATCH_SIZE):
            ...
            for ts_code, ok, err in results:
                if ok:
                    redis_client.sadd(DAILY_BASIC_FULL_HISTORY_PROGRESS_KEY, ts_code)
                    success_count += 1
                else:
                    error_count += 1
                    if err:
    ...
    final_done = len(redis_client.smembers(DAILY_BASIC_FULL_HISTORY_PROGRESS_KEY))
    if final_done >= total:
        redis_client.delete(DAILY_BASIC_FULL_HISTORY_PROGRESS_KEY)
        logger.info("✅ 每日指标全量历史同步完成，进度已清除")
