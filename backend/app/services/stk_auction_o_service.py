...
class StkAuctionOService(TushareSyncBase):
    """股票开盘集合竞价服务"""

    TABLE_KEY = 'stk_auction_o'
    FULL_HISTORY_START_DATE = '20190101'
    FULL_HISTORY_PROGRESS_KEY = 'sync:stk_auction_o:full_history:progress'
    ...
    async def sync_incremental(
        ...
        return await self.sync_stk_auction_o(
    ...
    async def sync_stk_auction_o(
        ...
        return await self.run_incremental_sync(
            fetch_fn=provider.get_stk_auction_o,
            upsert_fn=self.stk_auction_o_repo.bulk_upsert,
    ...
    async def get_latest_data(self) -> Dict[str, Any]:
        """获取最新的开盘集合竞价数据"""
        try:
            latest_date = await asyncio.to_thread(self.stk_auction_o_repo.get_latest_trade_date)
            if not latest_date:
