...
class MoneyflowMktDcService:
    ...
    async def get_suggested_start_date(self) -> Optional[str]:
        ...
        cfg = await asyncio.to_thread(SyncConfigRepository().get_by_table_key, 'moneyflow_mkt_dc')
        ...
        last_end = await asyncio.to_thread(
            self.sync_history_repo.get_last_end_date, 'moneyflow_mkt_dc', 'incremental'
        )
    ...
    async def sync_incremental(
        ...
        logger.info(f"[moneyflow_mkt_dc] 增量同步 start_date={start_date} end_date={end_date}")

        history_id = await asyncio.to_thread(
            self.sync_history_repo.create,
            'moneyflow_mkt_dc', 'incremental', sync_strategy or 'by_date_range', start_date,
        )
        try:
            ...
            result = {
                "status": "success",
                "records": records,
                "message": f"成功同步 {records} 条大盘资金流向数据"
            }
