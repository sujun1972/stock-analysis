...
class ModelPredictor:
    ...
    async def _predict_from_task(
        self, task_id: str, symbol: str, start_date: str, end_date: str
    ) -> Dict[str, Any]:
        """
        从任务信息进行预测
    ...
    async def _predict_from_path(
        self, model_path: str, config: Dict[str, Any], symbol: str, start_date: str, end_date: str
    ) -> Dict[str, Any]:
        """
        从模型路径进行预测
    ...
    async def _execute_prediction(
        ...
        for i, date in enumerate(dates):
            ...
            predictions_formatted.append(
                {"date": date_str, "prediction": pred_value, "actual": actual_value}
            )
        ...
        # 构建预测结果
        result = {"predictions": predictions_formatted, "metrics": metrics}
    ...
    async def _prepare_data(
        self, symbol: str, start_date: str, end_date: str, config: Dict[str, Any]
    ) -> tuple:
        """
        准备预测数据
    ...
    async def batch_predict(
        ...
        for symbol in symbols:
            try:
                ...
                errors.append({"symbol": symbol, "error": str(e), "error_code": "UNKNOWN"})
    ...
    async def predict_from_task(
        self, task_info: Dict[str, Any], symbol: str, start_date: str, end_date: str
    ) -> Dict[str, Any]:
        """
        向后兼容：使用任务信息进行预测
