...
class SuspendService:
    ...
    async def get_suggested_start_date(self) -> Optional[str]:
        ...
        cfg = await asyncio.to_thread(SyncConfigRepository().get_by_table_key, 'suspend')
        ...
        last_end = await asyncio.to_thread(
            self.sync_history_repo.get_last_end_date, 'suspend', 'incremental'
        )
    ...
    async def sync_incremental(
        ...
        logger.info(f"[suspend] 增量同步 start_date={start_date} end_date={end_date}")

        history_id = await asyncio.to_thread(
            self.sync_history_repo.create,
            'suspend', 'incremental', sync_strategy or 'by_date_range', start_date,
        )
        try:
            result = await self.sync_suspend(start_date=start_date, end_date=end_date)
