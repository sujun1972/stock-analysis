...
class DcIndexService:
    ...
    async def get_suggested_start_date(self) -> Optional[str]:
        ...
        cfg = await asyncio.to_thread(SyncConfigRepository().get_by_table_key, 'dc_index')
        ...
        last_end = await asyncio.to_thread(
            self.sync_history_repo.get_last_end_date, 'dc_index', 'incremental'
        )
    ...
    async def sync_incremental(self, start_date=None, end_date=None, sync_strategy=None, max_requests_per_minute=None) -> Dict:
        ...
        history_id = await asyncio.to_thread(
            self.sync_history_repo.create, 'dc_index', 'incremental', 'snapshot', None,
        )
        try:
            result = await self.sync_dc_index()
