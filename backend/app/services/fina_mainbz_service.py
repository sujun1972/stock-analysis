...
class FinaMainbzService:
    ...
    async def get_suggested_start_date(self) -> Optional[str]:
        ...
        last_end = await asyncio.to_thread(
            self.sync_history_repo.get_last_end_date, 'fina_mainbz', 'incremental'
        )
    ...
    async def sync_incremental(
        ...
        logger.info(f"[fina_mainbz] 增量同步 start_date={start_date} end_date={end_date}")

        history_id = await asyncio.to_thread(
            self.sync_history_repo.create,
            'fina_mainbz', 'incremental', sync_strategy or 'by_date_range', start_date,
        )

        try:
            result = await self.sync_fina_mainbz(start_date=start_date, end_date=end_date)
