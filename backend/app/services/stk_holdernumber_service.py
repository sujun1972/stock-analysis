...
class StkHolderNumberService(TushareSyncBase):
    """股东人数服务"""
    ...
    async def sync_full_history(
        ...
        """全量同步股东人数历史数据（支持 Redis 续继）

        stk_holdernumber 单次上限5500条，年数据量接近上限，按月切片避免截断。
        ...
        api_limit = (cfg.get('api_limit') or 5500) if cfg else 5500
        provider = self._get_provider(max_requests_per_minute)

        return await self.run_full_sync(
            redis_client=redis_client,
            fetch_fn=provider.get_stk_holdernumber,
            upsert_fn=self.stk_holdernumber_repo.bulk_upsert,
    ...
    async def sync_incremental(
        ...
        return await self.sync_stk_holdernumber(
            start_date=start_date,
            end_date=end_date,
    ...
    async def get_latest_by_code(self, ts_code: str, limit: int = 10) -> Dict[str, Any]:
        """获取指定股票的最新股东人数数据"""
        try:
    ...
    async def get_suggested_start_date(self) -> Optional[str]:
        ...
        if last_end and last_end < candidate:
            suggested = last_end
            logger.debug(
                f"[stk_holdernumber] 建议起始={suggested}（上次结束={last_end} < 候选={candidate}）"
            )
        else:
            suggested = candidate
            logger.debug(
                f"[stk_holdernumber] 建议起始={suggested}（候选={candidate}，上次结束={last_end}）"
            )
