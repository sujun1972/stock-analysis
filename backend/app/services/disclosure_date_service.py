...
class DisclosureDateService:
    ...
    async def sync_incremental(self, start_date=None, end_date=None, sync_strategy=None, max_requests_per_minute=None) -> Dict:
        """增量同步（按 ann_date 逐日回看，拉取最新披露公告）"""
        if start_date is None:
            start_date = await self.get_suggested_start_date()
        if end_date is None:
            end_date = datetime.now().strftime('%Y%m%d')

        logger.info(f"[disclosure_date] 增量同步 ann_date 回看: {start_date} ~ {end_date}")

        history_id = await asyncio.to_thread(
            self.sync_history_repo.create, 'disclosure_date', 'incremental', 'by_date', start_date,
        )
        try:
            ...
            while cur <= d_end:
                ...
                result = await self.sync_disclosure_date(ann_date=date_str)
