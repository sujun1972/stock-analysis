...
class MoneyflowSyncService(BaseSyncService):
    ...
    async def sync_moneyflow_stock_dc_full_history(
        self,
        redis_client,
        start_date: Optional[str] = None,
        concurrency: int = 5,
        ...
        策略：仅支持 ts_code 维度的逐只请求（DC 接口按日期全市场拉取单次上限 6000 条，
        历史年份数据极易截断，因此改为逐只请求保证完整性）。

        数据起始：20230911（Tushare DC 接口最早可提供）

        支持中断续继：Redis Set 记录已完成的 ts_code，重新触发时自动跳过。

        Redis Key: sync:moneyflow_stock_dc:full_history:progress
        ...
        Returns:
            {"status": "success", "total": int, "success": int, "skipped": int, "errors": int}
        """
        PROGRESS_KEY = "sync:moneyflow_stock_dc:full_history:progress"
        CONCURRENCY = max(1, concurrency)
        BATCH_SIZE = 50
        effective_start = start_date or "20230911"
        today = datetime.now().strftime("%Y%m%d")

        stock_repo = StockBasicRepository()
        all_ts_codes = await asyncio.to_thread(stock_repo.get_listed_ts_codes, status='L')
        total = len(all_ts_codes)
        logger.info(f"[全量DC个股资金流向] 共 {total} 只上市股票")
        ...
        pending_codes = [c for c in all_ts_codes if c not in completed_set]
        skip_count = len(completed_set)
        logger.info(f"[全量DC个股资金流向] 已完成 {skip_count} 只，剩余 {len(pending_codes)} 只")
        ...
        for batch_start in range(0, len(pending_codes), BATCH_SIZE):
            ...
            for ts_code, ok, err in results:
                if ok:
                    ...
                    error_count += 1
                    logger.error(f"[全量DC个股资金流向] 同步 {ts_code} 失败（下次续继）: {err}")
            ...
            logger.info(
                f"[全量DC个股资金流向] 进度: {done}/{total} ({round(done/total*100,1)}%) "
                f"| 本次成功={success_count} 失败={error_count}"
            )
        ...
        if final_done >= total:
            redis_client.delete(PROGRESS_KEY)
            logger.info("[全量DC个股资金流向] ✅ 全部完成，进度已清除")
