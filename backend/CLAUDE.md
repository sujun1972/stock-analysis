...
**多专家并行分析**：`POST /api/stock-ai-analysis/generate-multi` 通过 `asyncio.gather` 并行调用多个专家，数据收集只做一次（`build_stock_prompt` 缓存）。可选 `include_cio=true` 串联 CIO Agent 综合决策。核心业务逻辑抽在 `batch_ai_analysis_service.run_multi_expert_for_stock()`，同时被 `batch_ai_analysis` Celery 任务（多只并发）复用。`DEFAULT_TEMPLATE_KEYS`（`batch_ai_analysis_service.py`）维护 `analysis_type → template_key` 映射。

**批量 AI 分析（异步）**：
- `POST /api/stock-ai-analysis/batch`：前端提交一组 `ts_codes`，后端 `stock_quote_cache.get_quotes_batch` 补齐股票名 → 提交 `tasks.batch_ai_analysis` Celery 任务 → 返回 `celery_task_id`
...
- `GET /api/stock-ai-analysis/batch/active/ts-codes`：返回当前用户所有活跃批量任务中尚未完成的 ts_code 集合，供股票列表页标记"分析中"徽标
...
- 默认股票层并发 `DEFAULT_STOCK_CONCURRENCY=4`（每只内部还有 3-4 个 LLM 并行，总并发 ~16）。端点 `BatchAnalysisRequest.concurrency`（1~8）可覆盖；clamp 在任务侧做
...
- `celery_task_history` 表扩展 4 个分桶字段（迁移 `116_add_batch_progress_to_celery_task_history.sql`）；明细列表放 `metadata.items`。`task_type='batch_ai_analysis'` 区分于其他任务类型
