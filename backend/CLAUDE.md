...
**接口能力适配指南**（AkShare 接口签名差异较大，按能力选 Service 骨架）：

| AkShare 接口特征 | Service 模板 | 代表示例 |
|-----------------|-------------|---------|
| 支持日期参数（按日/按区间） | `sync_incremental` 逐日循环 + `sync_full_history` 按日并发 + Redis Set 续继 | `stock_anns_sync_service` / `cctv_news_sync_service` |
| 不支持日期参数，只拉最近 N 条 | `sync_incremental` 一次抓全；`sync_full_history` 退化为单次增量（按钮兼容） | `news_flash_sync_service`（财新要闻） |
| 按 ts_code 拉个股数据 | 只做 `sync_by_stock(ts_code)`，走"被动同步"路径 | `news_flash_sync_service.sync_by_stock`（东财个股新闻） |
| 多指标汇聚（每接口返回完整历史序列） | Provider 注册表 `INDICATOR_FETCHERS`（mixin 级常量）+ Service `sync_incremental` 遍历 key 逐个 UPSERT；`sync_full_history` 退化为单次增量 | `macro_sync_service`（宏观经济指标，7 接口 × 10 indicator_code）|

**个股代码抽取（文本 → ts_code）**：从快讯/新闻标题/摘要中识别 A 股代码时，用 `app.services.news_anns.StockCodeExtractor`（正则 + `stock_basic` 白名单双校验，延迟加载且线程安全）。`.extract(text)` 返回去重 ts_code 列表；`.extract_from_items(items)` 就地为 dict 列表填充 `related_ts_codes`，合并已有（如 eastmoney 接口天然带 ts_code 的情况）与文本抽取结果。白名单覆盖上市/退市/暂停三种状态，历史快讯提及已退市代码也能命中。
...
**JSON 合法性校验 + 写库策略**：`extract_json_and_score()` 返回 `(cleaned_text, score, is_malformed)` 三元组。`is_malformed=True` 表示 `extract_json_text` 内的 `_try_repair_json` 修复链（缺 `}` / 缺 `]` / key 错位）覆盖不到的错误（典型：LLM 偶发输出"字符串值内未转义引号"）。**调用方按业务场景区分**：

- **CIO（强结构化展示）**：前端 `StructuredAnalysisContent` 直接解构 JSON 渲染，畸形即"无法显示"。`is_malformed=True` 必须**拒绝写库** —— 端点返回 `bad_request("CIO 输出 JSON 畸形且自动修复失败")`，多专家路径把它计入 `errors[]` 不写 `cio_result`。避免污染历史让用户翻到该版本时显示"内容解析失败"
- **普通专家（hot_money_view / midline / longterm / 复盘等）**：前端在 `safeParseJSON` 失败时降级到 `MarkdownContent` 兜底渲染，畸形 JSON **照常写库** + 仅 `logger.warning`，不阻断流程

新增专家类型时：① 决定走结构化展示还是 Markdown 兜底；② 解构 `is_malformed` 后照样判断；③ 共享同一条修复链，无需自己造 JSON 修复算法。前端 CIO 卡 [CioDecisionCard.tsx](../frontend/src/components/stocks/analysis/CioDecisionCard.tsx) 对历史遗留畸形数据保留两套空态分支：`!current` → "尚未生成"；`!parsed` → "内容解析失败"+查看源代码/翻页/删除入口。

**多专家并行分析（核心 service）**：`batch_ai_analysis_service.run_multi_expert_for_stock()` 通过 `asyncio.gather` 并行调用多个专家，数据收集只做一次（`build_stock_prompt` 缓存）；可选 `include_cio=true` 串联 CIO Agent。同时被批量任务（多只并发）和单只一键任务（concurrency=1）复用。`DEFAULT_TEMPLATE_KEYS`（`batch_ai_analysis_service.py`）维护 `analysis_type → template_key` 映射。

**同交易日专家报告复用**（`reuse_existing_experts: bool = True`，默认开）：`run_multi_expert_for_stock` 在确定 `trade_date` 后，对每个 `expert_types` 调 `StockAiAnalysisRepository.get_by_trade_date()` 查同日记录，**JSON 合法**则跳过 LLM 调用、把记录直接合入 `results` 让 CIO 拼装时拿到完整三专家上下文；JSON 畸形按缺失处理重新生成。**CIO 始终重跑**（综合性结论用户每次点"一键分析"都希望刷新，且重跑成本远低于三专家加起来）。同步配置：`GenerateMultiRequest` / `BatchAnalysisRequest` 都带 `reuse_existing_experts` 字段并透传到 `batch_ai_analysis_task` kwargs；前端不暴露 UI，默认 `True` 即可。

**AI 分析任务（异步 + 轮询）**：批量分析与单只一键分析共用同一个 Celery 任务 `tasks.batch_ai_analysis`，仅以 `task_type` 区分；前端协议完全一致。

- 端点：
  - `POST /api/stock-ai-analysis/batch`（批量）：提交一组 `ts_codes`，登记 `task_type='batch_ai_analysis'`
  - `POST /api/stock-ai-analysis/generate-multi`（单只一键）：`ts_codes=[一只] / concurrency=1`，登记 `task_type='single_ai_analysis'`
  - `GET /api/stock-ai-analysis/batch/{celery_task_id}`：查询进度（前端 3 秒轮询），返回 `items[]`（每只 `{ts_code, stock_name, status, error, duration_sec, expert_count}`）和 `total/completed/success/failed_items` 分桶计数
  - `GET /api/stock-ai-analysis/batch/active/ts-codes`：返回当前用户所有活跃 AI 分析任务（**两类 task_type 都覆盖**）中尚未完成的 ts_code 集合，供股票列表页标记"分析中"徽标
  - `GET /api/stock-ai-analysis/active/by-ts-code/{ts_code}`：查指定 ts_code 当前是否有活跃任务，返回 `celery_task_id`；用于 `/analysis` 页面刷新/导航后恢复"分析中"状态并续轮询
- Celery 任务 `tasks.batch_ai_analysis`（`batch_ai_analysis_tasks.py`）内部以 `asyncio.Semaphore(concurrency)` 并发调用 `run_multi_expert_for_stock`，每只完成后通过 `CeleryTaskHistoryRepository.update_batch_progress()` 回写 `progress / total_items / completed_items / success_items / failed_items / metadata.items[]`
- 单批上限 `MAX_BATCH_SIZE=200`（`batch_ai_analysis_tasks.py` 常量），端点 `submit_batch_analysis` 提交前强制校验，超过返回 400 提示分批；任务 `soft_time_limit=7200s` / `time_limit=7500s` 按 200 只 × ~30s 中位耗时 / 4 并发 ≈ 25 分钟设计，留 5x 安全裕度。修改上限时**必须同步上调**超时
- 默认股票层并发 `DEFAULT_STOCK_CONCURRENCY=4`（每只内部还有 3-4 个 LLM 并行，总并发 ~16）。端点 `BatchAnalysisRequest.concurrency`（1~8）可覆盖；clamp 在任务侧做。单只一键路径强制 `concurrency=1`
- **SoftTimeLimitExceeded 部分成功语义**：任务捕获该异常后，把所有 `pending`/`running` 条目标记为 `timeout_skipped`、计入 `failed_items` 分桶，再正常 return（task_success 信号写 `status='success'`）。前端 `metadata.items` 仍能看到完整明细，已成功的不会被误判为失败
- `celery_task_history` 表扩展 4 个分桶字段（迁移 `116_add_batch_progress_to_celery_task_history.sql`）；明细列表放 `metadata.items`。`task_type` 取值：`batch_ai_analysis`（批量）/ `single_ai_analysis`（单只一键）
- Celery 信号处理器只改 `status / started_at / completed_at / result / error / duration_ms`，与 `update_batch_progress` 的字段不冲突
- **task_success 与最后一次 _flush_progress 抢跑陷阱**：两者是独立 SQL，信号若先到，前端这一轮拿到的 `metadata.items` 还是上一帧（成功/失败计数与终态不一致）。**前端必须在检测到 status=success/failure 后再补拉一次**取最终 metadata 兜底（已在 `BatchAnalysisDialog` 与 `useMultiAnalysisTask` 实现）
- **提交顺序约束**：端点必须先 `uuid.uuid4()` 生成 `task_id` → `TaskHistoryHelper.create_task_record()` 写 DB → 再用 `apply_async(task_id=...)` 提交；否则 worker 可能在 DB 记录创建完成前就开始跑 `update_batch_progress()`，`WHERE celery_task_id=?` 不命中导致进度静默丢失
- `CeleryTaskHistoryRepository.get_active_batch_ts_codes(user_id, task_type)` 接受 `str` 或 `Sequence[str]`，多 task_type 时一次扫所有活跃记录；`get_active_task_id_by_ts_code(user_id, ts_code, task_type)` 是其单股反查变体
...
- 共享常量：`prompt_templates.EXPERT_OUTPUT_PLACEHOLDERS`（占位符列表）、`batch_ai_analysis_service.EXPERT_TYPE_TO_CIO_PLACEHOLDER`（映射）。新增 CIO 占位符必须同步两处。
