...
- Celery 任务 `tasks.batch_ai_analysis`（`batch_ai_analysis_tasks.py`）内部以 `asyncio.Semaphore(STOCK_CONCURRENCY=2)` 并发调用 `run_multi_expert_for_stock`，每只完成后通过 `CeleryTaskHistoryRepository.update_batch_progress()` 回写 `progress / total_items / completed_items / success_items / failed_items / metadata.items[]`
