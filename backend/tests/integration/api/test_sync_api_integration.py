...
class TestSyncDailyBatch:
    ...
    async def test_sync_daily_batch_success(self):
        """测试成功批量同步日线数据"""
        # Arrange
        request = SyncDailyBatchRequest(codes=["000001", "000002"], years=5)
        mock_result = {"total": 2, "success": 2, "failed": 0, "aborted": False}
    ...
    async def test_sync_daily_batch_aborted(self):
        ...
        mock_result = {"total": 2, "success": 1, "failed": 0, "aborted": True}
...
class TestSyncRealtimeQuotes:
    ...
    async def test_sync_realtime_quotes_success(self):
        ...
        request = SyncRealtimeRequest(
            codes=["000001", "000002"], batch_size=100, update_oldest=False
        )
        mock_result = {"total": 2, "success": 2, "failed": 0, "partial_success": False}
    ...
    async def test_sync_realtime_quotes_partial_success(self):
        ...
        mock_result = {"total": 2, "success": 1, "failed": 1, "partial_success": True}
