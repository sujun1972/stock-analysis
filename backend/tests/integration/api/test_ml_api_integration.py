...
class TestListModels:
    ...
    async def test_list_models_success(self):
        ...
        # Mock 数据库查询
        mock_db_results = [
            (1, None, 'exp_1', 'task_123', {'symbol': '000001', 'model_type': 'lightgbm'},
             {'rmse': 0.05, 'ic': 0.08}, None, None, '/path/to/model1.pkl',
             datetime(2024, 1, 1), None, datetime(2024, 1, 1))
        ]

        with patch('app.api.endpoints.ml.get_database') as mock_get_db, \
             patch('app.api.endpoints.ml.asyncio.to_thread', new=AsyncMock(return_value=[[2], mock_db_results])):
            mock_db = Mock()
...
class TestGetFeatureSnapshot:
    ...
    async def test_get_feature_snapshot_success(self):
        ...
        with patch('app.api.endpoints.ml.Path.exists', return_value=True), \
