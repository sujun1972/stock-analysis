...
class TestMarketStatusAPI:
    ...
    def test_get_market_status_trading(self, mock_adapter):
        """测试获取市场状态 - 交易时段"""
        # Arrange
        mock_adapter.get_market_status = AsyncMock(return_value=("trading", "交易中（早盘）"))
        mock_adapter.get_next_trading_session = AsyncMock(
            return_value=(datetime(2023, 12, 29, 13, 0, 0), "今日午盘开盘")
        )
    ...
    def test_get_market_status_closed(self, mock_adapter):
        """测试获取市场状态 - 休市"""
        # Arrange
        mock_adapter.get_market_status = AsyncMock(return_value=("closed", "休市（周末或节假日）"))
        mock_adapter.get_next_trading_session = AsyncMock(
            return_value=(datetime(2023, 12, 31, 9, 15, 0), "下一交易日集合竞价")
        )
        mock_adapter.MORNING_OPEN = time(9, 30)
        mock_adapter.MORNING_CLOSE = time(11, 30)
        mock_adapter.AFTERNOON_OPEN = time(13, 0)
    ...
    def test_get_market_status_call_auction(self, mock_adapter):
        """测试获取市场状态 - 集合竞价"""
        # Arrange
        mock_adapter.get_market_status = AsyncMock(return_value=("call_auction", "集合竞价"))
        mock_adapter.get_next_trading_session = AsyncMock(
            return_value=(datetime(2023, 12, 29, 9, 30, 0), "今日早盘开盘")
        )
        mock_adapter.MORNING_OPEN = time(9, 30)
        mock_adapter.MORNING_CLOSE = time(11, 30)
        mock_adapter.AFTERNOON_OPEN = time(13, 0)
...
class TestRefreshCheckAPI:
    ...
    def test_check_refresh_needed_should_refresh(self, mock_adapter):
        ...
        # Arrange
        mock_adapter.get_market_status = AsyncMock(return_value=("trading", "交易中（早盘）"))
        mock_adapter.should_refresh_realtime_data = AsyncMock(return_value=(True, "实时行情更新"))
    ...
    def test_check_refresh_needed_force(self, mock_adapter):
        ...
        # Arrange
        mock_adapter.get_market_status = AsyncMock(return_value=("closed", "休市（周末或节假日）"))
        mock_adapter.should_refresh_realtime_data = AsyncMock(return_value=(True, "用户强制刷新"))
    ...
    def test_check_refresh_needed_with_codes(self, mock_adapter):
        """测试检查刷新 - 指定股票代码"""
        # Arrange
        mock_adapter.get_market_status = AsyncMock(return_value=("trading", "交易中（午盘）"))
        mock_adapter.should_refresh_realtime_data = AsyncMock(return_value=(True, "实时行情更新"))

        # Act
        response = client.get("/api/market/refresh-check?codes=000001.SZ&codes=600000.SH")
    ...
    def test_check_refresh_needed_no_refresh(self, mock_adapter):
        """测试检查刷新 - 不需要刷新"""
        # Arrange
        mock_adapter.get_market_status = AsyncMock(return_value=("after_hours", "盘后"))
...
class TestNextSessionAPI:
    ...
    def test_get_next_session_lunch_break(self, mock_adapter):
        ...
        mock_adapter.get_next_trading_session = AsyncMock(return_value=(next_time, "今日午盘开盘"))
        mock_adapter.get_market_status = AsyncMock(return_value=("closed", "午间休市"))
...
class TestErrorHandling:
    ...
    def test_get_market_status_error(self, mock_adapter):
        """测试获取市场状态异常"""
        # Arrange
        mock_adapter.get_market_status = AsyncMock(side_effect=Exception("数据库连接失败"))
    ...
    def test_get_trading_info_error(self, mock_adapter):
        """测试获取交易信息异常"""
        # Arrange
        mock_adapter.is_trading_day = AsyncMock(side_effect=Exception("服务不可用"))
    ...
    def test_check_refresh_needed_error(self, mock_adapter):
        """测试检查刷新异常"""
        # Arrange
        mock_adapter.get_market_status = AsyncMock(side_effect=Exception("网络错误"))
    ...
    def test_get_next_session_error(self, mock_adapter):
        """测试获取下一交易时段异常"""
        # Arrange
        mock_adapter.get_next_trading_session = AsyncMock(side_effect=Exception("计算错误"))
