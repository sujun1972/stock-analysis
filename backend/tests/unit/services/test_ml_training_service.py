...
class TestCreateTask:
    ...
    async def test_create_task_success(self):
        ...
        with patch.object(service.task_manager, 'create_task', new=AsyncMock(return_value=mock_task_id)):
            # Act
...
class TestRunTraining:
    ...
    async def test_run_training_task_not_found(self):
        ...
        with patch.object(service.task_manager, 'run_training', new=AsyncMock(side_effect=ValueError("任务不存在"))):
            # Act & Assert
...
class TestListTasks:
    ...
    def test_list_tasks_with_pagination(self):
        ...
        mock_tasks_response = {
            'tasks': [
                {'task_id': f'task_{i}', 'status': 'completed'}
...
class TestCancelTask:
    ...
    def test_cancel_task_invalid_status(self):
        ...
        with patch.object(service.task_manager, 'cancel_task', side_effect=ValueError("任务无法取消")):
            # Act & Assert
...
class TestDeleteTask:
    ...
    def test_delete_task_not_found(self):
        ...
        with patch.object(service.task_manager, 'delete_task', side_effect=ValueError("任务不存在")):
            # Act & Assert
            with pytest.raises(ValueError, match="任务不存在"):
                service.delete_task(task_id)
...
class TestPredict:
    ...
    async def test_predict_with_model_id(self):
        ...
        with patch.object(service.predictor, 'predict', new=AsyncMock(return_value=mock_predictions)):
            # Act
...
class TestBatchPredict:
    ...
    async def test_batch_predict_success(self):
        ...
        with patch.object(service.predictor, 'batch_predict', new=AsyncMock(return_value=mock_batch_results)):
            # Act
