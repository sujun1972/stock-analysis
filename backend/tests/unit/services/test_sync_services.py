...
class TestSyncSingleStock:
    ...
    async def test_sync_single_stock_success(self):
        ...
        with (
            ...
            patch(
                "app.services.daily_sync_service.DataProviderFactory.create_provider"
...
class TestSyncBatch:
    ...
    async def test_sync_batch_with_codes(self):
        ...
        with (
            ...
            patch.object(
                service.config_service, "check_sync_abort_flag", new=AsyncMock(return_value=False)
            ),
            patch("app.services.daily_sync_service.DataProviderFactory.create_provider"),
            patch("asyncio.to_thread", new=AsyncMock(side_effect=mock_to_thread_side_effect)),
        ):
    ...
    async def test_sync_batch_abort(self):
        ...
        with (
            ...
            patch.object(
                service.config_service,
                "check_sync_abort_flag",
                new=AsyncMock(side_effect=abort_calls),
            ),
        ):
...
class TestSyncStockList:
    ...
    async def test_sync_stock_list_success(self):
        ...
        f"stock_list_{datetime.now().strftime('%Y%m%d%H%M%S')}"
        ...
        with (
            ...
            patch.object(service.config_service, "update_sync_status", new=AsyncMock()),
            patch(
                "app.services.stock_list_sync_service.DataProviderFactory.create_provider"
            ) as mock_provider_factory,
...
class TestSyncNewStocks:
    ...
    async def test_sync_new_stocks_success(self):
        ...
        mock_config = {"data_source": "akshare", "tushare_token": ""}
        mock_new_stocks = pd.DataFrame({"code": ["301234", "301235"], "name": ["新股A", "新股B"]})
...
class TestSyncMinuteData:
    ...
    async def test_sync_minute_data_success(self):
        ...
        with (
            ...
            patch(
                "app.services.realtime_sync_service.DataProviderFactory.create_provider"
            ) as mock_provider_factory,
...
class TestSyncRealtimeQuotes:
    ...
    async def test_sync_realtime_quotes_with_codes(self):
        ...
        mock_df = pd.DataFrame(
            {"code": codes, "name": ["平安银行", "万科A"], "price": [10.0, 20.0]}
        )
    ...
    async def test_update_sync_status_success(self):
        ...
        with (
            ...
            result = await manager.update_sync_status(
                status="running", progress=50, total=100, completed=50
            )
