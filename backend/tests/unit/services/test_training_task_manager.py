...
class TestTrainingTaskManagerInit:
    ...
    def test_load_existing_metadata(self):
        ...
        with (
            patch("app.services.training_task_manager.DatabaseManager"),
...
class TestRunTraining:
    ...
    async def test_run_training_failure(self):
        ...
        with patch("app.services.training_task_manager.DatabaseManager"):
            ...
            with patch.object(
                manager, "_run_training", new=AsyncMock(side_effect=Exception("训练失败"))
            ):
                # Act & Assert
...
class TestRunSingleStockTraining:
    ...
    async def test_run_single_stock_training_success(self):
        ...
        with patch("app.services.training_task_manager.DatabaseManager"):
            ...
            with (
                patch("app.services.training_task_manager.CoreTrainingService") as mock_core,
...
class TestRunPooledTraining:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="PooledTrainingPipeline在函数内部动态导入，需要重构测试以mock src.data_pipeline.pooled_training_pipeline"
    )
    async def test_run_pooled_training_success(self):
        ...
        with patch("app.services.training_task_manager.DatabaseManager"):
            ...
            with (
                patch("app.services.training_task_manager.PooledTrainingPipeline") as mock_pipeline,
                patch("asyncio.to_thread", new=AsyncMock(return_value=mock_pipeline_result)),
                patch.object(
                    manager, "_save_pooled_experiment_to_db", new=AsyncMock(return_value=None)
                ),
            ):
                # Act
...
class TestGetTask:
    ...
    def test_get_task_success(self):
        ...
        with patch("app.services.training_task_manager.DatabaseManager"):
            ...
            manager.tasks[task_id] = {"task_id": task_id, "status": "completed", "progress": 100}
...
class TestCancelTask:
    ...
    def test_cancel_task_success(self):
        ...
        with patch("app.services.training_task_manager.DatabaseManager"):
            ...
            task_id = "test_task"
            manager.tasks[task_id] = {"task_id": task_id, "status": "pending"}
    ...
    def test_cancel_task_invalid_status(self):
        ...
        with patch("app.services.training_task_manager.DatabaseManager"):
            ...
            task_id = "test_task"
            manager.tasks[task_id] = {"task_id": task_id, "status": "completed"}
...
class TestSaveAndLoadMetadata:
    ...
    def test_save_metadata_success(self):
        ...
        with patch("app.services.training_task_manager.DatabaseManager"):
            manager = TrainingTaskManager()
            manager.tasks = {"task_1": {"task_id": "task_1", "status": "completed"}}
