...
def mock_query_manager():
    """模拟查询管理器"""
    mock = Mock()
    mock.get_stock_list = Mock(return_value=[
        {'code': '000001', 'name': '平安银行', 'market': '主板'},
        {'code': '000002', 'name': '万科A', 'market': '主板'}
    ])
    mock.load_daily_data = Mock(return_value=pd.DataFrame({
        'open': [10.0, 10.5],
        'high': [10.5, 11.0],
        'low': [9.8, 10.2],
        'close': [10.2, 10.8],
    ...
    mock.load_minute_data = Mock(return_value=pd.DataFrame({
        'time': ['09:30', '09:31'],
        'open': [10.0, 10.1],
        'close': [10.1, 10.2],
        'volume': [10000, 11000]
    }))
    mock.check_daily_data_completeness = Mock(return_value={
        'total_days': 100,
        'missing_days': 5,
        'completeness': 0.95
    })
...
def data_adapter(mock_pool_manager, mock_query_manager, mock_insert_manager):
    """创建数据适配器实例"""
    with patch('app.core_adapters.data_adapter.ConnectionPoolManager', return_value=mock_pool_manager), \
