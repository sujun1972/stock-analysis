...
| 交易所重点提示证券 | `/reference-data/stk-alert` | 全量策略可配置（默认 by_month，支持 snapshot），增量按日期范围 |
| 股权质押统计 | `/reference-data/pledge-stat` | 全量策略可配置（默认 by_ts_code，支持 snapshot），增量 by_ts_code |
...
| 类型 | 特征 | 策略 | 代表接口 |
|------|------|------|---------|
| **快照接口（snapshot）** | 不支持日期范围；不传日期直接返回全量当前数据；传日期参数返回 0 条 | `strategy='snapshot'`：不传任何日期，仅 `limit`/`offset` 分页 | 真正的快照接口（目前三个参考数据接口实测支持日期范围，故默认 `by_month`；若发现传日期返回 0 条，可在 sync_configs 改为 `snapshot`） |
| **日期范围接口** | 支持 `start_date`/`end_date`，但单次有上限（300~6000条） | 按月切片（`by_month`），5并发，Redis Set续继（月起始日期为 key）；数据量小时可用 `by_quarter` | `block_trade`（1000/次）、`stk_holdertrade`（~3000/年）、`repurchase`（~1000/年）、`share_float`（~6000/年）、`stk_holdernumber`（~5500/年）、`stk_shock`、`stk_high_shock`、`stk_alert` |
| **仅支持 trade_date** | 文档声称支持日期范围，但实测只传 `start_date`/`end_date` 报"ts_code,trade_date至少输入一个参数"；单日数据量大（每日约7000+条，需翻页） | 逐日请求（`by_date` + `date_param='trade_date'`），5并发，Redis续继（以日期为 key） | `ccass_hold_detail` |
| **仅支持 ts_code** | 无法按日期拉全市场；必须逐只股票请求 | 逐只请求（`by_ts_code`），5并发，Redis Set续继（ts_code 为 key） | `pledge_stat`（默认）、财务报表系列、`cyq_chips`、`cyq_perf`（SDK 校验：ts_code 或 trade_date 至少一个，web API 无此限制；Service 层强制覆盖为 by_ts_code） |
...
**定时任务以空参数触发导致 Tushare 校验失败**：

部分 Tushare 接口（如 `ccass_hold_detail`、`stk_nineturn`）要求 `ts_code` 或日期至少传一个，否则报 `"xxx,trade_date至少输入一个参数"` 或 `"股票代码和日期至少输入一个参数"`。定时任务以空 `{}` 触发时，`run_incremental_sync` 进入 `else` 分支（无切片），直接以空参数调用 Tushare，触发此校验。

**修复模式（在 Service 的增量同步方法开头加）**：

```python
async def sync_xxx(self, ts_code=None, trade_date=None, start_date=None, ...):
    ...
    if not ts_code and not trade_date and not effective_start:
        # 无任何过滤参数（定时任务触发场景），自动计算起始日期以触发切片策略
        effective_start = await self.get_suggested_start_date()
        logger.info(f"xxx 增量同步：未传 ts_code/trade_date/start_date，自动使用建议起始日期 {effective_start}")
...
**判断哪些接口需要此处理**：日志出现 `strategy=None start=None end=(不限)` 且随后 Tushare 报参数校验失败，即可确认需要此修复。

**已修复的接口**：`ccass_hold_detail`（`by_date`）、`stk_nineturn`（`by_date_range`）

**仅支持 trade_date 的接口**：文档声称支持 `start_date`/`end_date`，但实测报"ts_code,trade_date至少输入一个参数"。使用 `by_date` + `date_param='trade_date'`，`TushareSyncBase` 每片只传 `trade_date=YYYYMMDD`（取切片的起始日，两端相同）：

```python
return await self.run_full_sync(
    ...
    strategy='by_date',
    date_param='trade_date',   # 每片传 trade_date=YYYYMMDD，而非 start_date/end_date
)
```

**`TushareSyncBase` 继承模式（特色数据服务标准）**：

所有继承 `TushareSyncBase` 的服务必须定义三个类常量，并将 `FULL_HISTORY_PROGRESS_KEY` 传给 `run_full_sync`：

```python
class YourService(TushareSyncBase):
    TABLE_KEY = 'your_table'
    FULL_HISTORY_START_DATE = '20180101'
    FULL_HISTORY_PROGRESS_KEY = 'sync:your_table:full_history:progress'  # 类常量，非实例变量
    ...
    async def sync_full_history(self, redis_client, start_date=None, concurrency=5,
        ...
        return await self.run_full_sync(
            ...
            progress_key=self.FULL_HISTORY_PROGRESS_KEY,  # 使用类常量
            strategy=strategy,
...
已采用此模式的服务：`cyq_chips`、`cyq_perf`、`report_rc`、`ccass_hold`、`ccass_hold_detail`、`hk_hold`、`stk_auction_o`、`stk_auction_c`、`stk_nineturn`、`stk_ah_comparison`、`stk_surv`、`stk_shock`、`stk_high_shock`、`stk_alert`、`pledge_stat`

**`run_full_sync` 支持的策略**：`by_month`（默认）、`by_week`、`by_date`、`by_quarter`、`by_ts_code`、`snapshot`。
- `snapshot` — 不传任何日期，仅用 `limit`/`offset` 分页拉取接口返回的全量当前数据；适用于真正的快照接口（传日期返回 0 条）
- `by_quarter` — 按自然季度切片；适用于数据量较小但需要历史覆盖的接口

**`strategy` 必须由 API 端点从 `sync_configs.full_sync_strategy` 读取后传给 Celery task，再由 task 传给 `service.sync_full_history(strategy=strategy)`。** Service 层的 `strategy` 参数仅作为兜底默认值，不应在 service 内部直接读配置。

**全量同步并发数从 `sync_configs` 读取**：

全量同步 API 端点不硬编码并发数，而是从 `sync_configs.full_sync_concurrency` 读取，允许管理员在同步配置页面调整：

```python
@router.post("/sync-full-history")
async def sync_full_history_async(
...
```python
async def sync_full_history(self, redis_client, start_date=None, concurrency: int = 5, update_state_fn=None):
...
```python
MAX_OFFSET = 100_000  # Tushare offset 上限为 100,000（offset=100001 报错），最大允许值为 100,000
...
| CCASS持股明细 | `/features/ccass-hold-detail` | 8000积分/次；增量同步支持 ts_code/start_date/end_date；全量同步逐日请求（by_date + trade_date），每日约7000+条需翻页 |
...
**全量历史同步任务必须加 `acks_late=False`**：

全量历史同步任务（`sync_*_full_history`）耗时数小时，支持 Redis Set 续继。必须在装饰器中设置 `acks_late=False`，否则 worker 重启后任务消息会重新入队，产生重复执行：
...
- 全局配置 `task_acks_late=True` 确保普通短任务在 worker 崩溃时重试
- 全量历史任务中断后通过手动重新触发 + Redis 续继恢复，不需要 Celery 自动重投递
- 不加此参数 → worker 重启 → 同一任务并发运行两份 → 重复请求 Tushare 浪费积分
...
**问题1b: "server closed the connection unexpectedly" — Repository 查询在 Celery worker 中失败**
- **原因**: `DatabaseManager` 是单例，在主进程中创建。Celery `fork()` 出的 worker 继承了父进程已损坏的连接池，首次查询时连接已断开
- **解决**: `celery_signals.py` 的 `worker_process_init` 信号处理器在每个 fork worker 启动时调用 `DatabaseManager.reset_instance()`，强制子进程在首次使用时重建自己的连接池
- **实现位置**: `backend/app/celery_signals.py` — `worker_process_init_handler`
- **注意**: 此信号在 worker **进程**初始化时触发（非任务级别），每个 worker 进程只触发一次
...
- **解决**：将 Service 实例化移入任务函数体（延迟导入），每次任务执行都创建新实例。**禁止**在任务文件顶层创建 Service 实例：
  ...
  ```
- **已修复文件**：`moneyflow_tasks.py`（2026-04-08）；`moneyflow_mkt_dc_tasks.py`、`moneyflow_hsgt_tasks.py`（2026-04-06）；`moneyflow_stock_dc_tasks.py` 等（更早）
...
- **解决**：`TushareErrorMessages.is_rate_limit_error()` 已修正，不再包含 `QUERY_FAILED_GENERIC`，此类错误现在按普通错误处理（短暂重试后报错）。同时，所有分页循环须加 `MAX_OFFSET = 100_000` 上限保护，在接近 Tushare offset 上限前主动停止分页。
