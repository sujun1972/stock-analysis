...
| **仅支持 ts_code** | 无法按日期拉全市场；必须逐只股票请求 | 逐只请求（`by_ts_code`），5并发，Redis Set续继（ts_code 为 key） | `pledge_stat`（默认）、财务报表系列 |
...
已采用此模式的服务：`cyq_chips`、`cyq_perf`、`ccass_hold`、`ccass_hold_detail`、`hk_hold`、`stk_auction_o`、`stk_auction_c`、`stk_nineturn`、`stk_ah_comparison`、`stk_surv`、`stk_shock`、`stk_high_shock`、`stk_alert`、`pledge_stat`
