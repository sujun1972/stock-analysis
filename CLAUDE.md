...
**`ai_output_parser.extract_json_text` 含 JSON 自动修复**：两级修复策略——① 中间修复（`_repair_misplaced_keys`）：通过深度追踪检测应在顶层的 key 被嵌入子对象时，在其前方插入缺失的 `}`；② 末尾补全（`_repair_trailing`）：AI 截断导致缺少 1-3 个闭合 `}` / `]` 时自动补全。`_EXPECTED_TOP_KEYS` 集合定义了专家分析 JSON 的顶层字段名。修复链覆盖不到的错误（典型：LLM 偶发输出"字符串值内未转义引号"）会让 `extract_json_and_score` 返回 `is_malformed=True`，调用方按业务场景区分：CIO 强结构化展示**拒绝写库**避免污染历史，普通专家走 `MarkdownContent` 兜底**仅日志告警**。详见 [backend/CLAUDE.md "JSON 合法性校验 + 写库策略"](backend/CLAUDE.md)。
