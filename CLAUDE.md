...
| **仅支持 trade_date** | 接口文档声称支持日期范围，但实际传 `start_date`/`end_date` 会报"ts_code,trade_date至少输入一个参数" | 逐日请求（`by_date` + `date_param='trade_date'`），5并发，Redis续继 | `ccass_hold_detail` |
...
4. 若步骤 2 报 "ts_code,trade_date至少输入一个参数"（即使文档声称支持日期范围），则该接口属于**仅支持 trade_date** 类型，使用 `by_date` + `date_param='trade_date'`
...
**`date_param` 参数（仅支持 trade_date 的接口）**：

`TushareSyncBase.run_full_sync()` 支持 `date_param` 参数。当接口不接受 `start_date`/`end_date` 但接受单日 `trade_date` 时，配合 `by_date` 策略逐日请求：

```python
# ccass_hold_detail 示例：接口要求 ts_code 或 trade_date 至少一个
return await self.run_full_sync(
    ...
    strategy='by_date',       # 逐日切片
...
已采用此模式的服务：`cyq_chips`、`cyq_perf`、`ccass_hold`、`ccass_hold_detail`、`hk_hold`、`stk_auction_o`、`stk_auction_c`、`stk_nineturn`、`stk_ah_comparison`、`stk_surv`
