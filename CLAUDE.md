...
| **日期范围接口** | 支持 `start_date`/`end_date`，但单次有上限（300~6000条） | 按月切片，5并发，Redis Set续继（月起始日期为 key） | `block_trade`（1000/次）、`stk_holdertrade`（~3000/年）、`repurchase`（~1000/年）、`share_float`（~6000/年）、`stk_holdernumber`（~5500/年）、`ccass_hold_detail` |
...
| CCASS持股明细 | `/features/ccass-hold-detail` | 8000积分/次；支持 ts_code/start_date/end_date 可选查询；全量同步按月切片 |
