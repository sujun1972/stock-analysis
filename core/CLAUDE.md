...
1. **空结果降级**：部分接口（如 `stock_notice_report` / `stock_individual_notice_report`）在区间内无数据时会抛 `KeyError('代码')`（内部 pandas 空 DF 索引失败）。Mixin 方法应把它识别为"区间内无数据"的**正常语义**，降级为 `Response.warning(data=空 DataFrame)`，而非当作错误抛出。

2. **pyarrow 字符串引擎 regex 错误**：AkShare 部分接口（如 `stock_news_em`）内部用 `df.str.replace(r"\u3000", ...)` 清洗。pandas 在 pyarrow string backend 下把 `\u3000` 送进 Arrow regex 引擎，后者不支持 `\u` 转义，抛 `pyarrow.lib.ArrowInvalid: Invalid regular expression`。绕过方案：在 Mixin 方法中用 `pd.option_context('future.infer_string', False, 'mode.string_storage', 'python')` 包裹 AkShare 调用。`news_and_anns.py` 的 `_call_and_wrap(call_context=...)` 参数已封装此能力，新方法直接传 `call_context=lambda: pd.option_context(...)` 即可。

3. **多 schema 复用 `_call_and_wrap`**：Mixin 中承载不同输出 schema（公告 / 快讯 / 联播等）时，`_call_and_wrap` 的 `normalizer` + `empty_factory` 参数允许不同方法各自传入标准化函数与空 DataFrame 工厂，不需要为每个数据域复制一份错误处理样板。参考 `news_and_anns.py` 的 `get_news_flash` / `get_cctv_news` 用法。
