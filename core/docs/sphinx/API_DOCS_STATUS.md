...
2. **验证文档质量**
   ```bash
   # 构建并检查警告数量
   ./build.sh 2>&1 | grep "WARNING" | wc -l
   # 目标: 从196个警告降至 < 10个
   ```
...
| 指标 | 当前值 | 目标值 | 优先级 |
|------|--------|--------|--------|
| 导入成功率 | 0.5% (1/197) | 95%+ (187/197) | P0 |
| 构建警告数 | 196 | < 10 | P1 |
| 文档完整度 | 20% (仅结构) | 90%+ (含内容) | P1 |
| Docstring 覆盖率 | 80% | 95%+ | P2 |
...
- [ ] Issue #1: 修复 data_pipeline ↔ pipeline 循环导入
