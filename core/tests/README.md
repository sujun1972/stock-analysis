# Core 项目测试套件

完整的单元测试套件，覆盖所有重构后的模块化组件。

## 📋 测试覆盖

### 组件级测试

| 测试模块 | 测试文件 | 覆盖组件 | 测试用例数 |
|---------|---------|---------|-----------|
| DataLoader | `test_data_loader.py` | 数据加载器 | 9 |
| FeatureEngineer | `test_feature_engineer.py` | 特征工程器 | 10 |
| DataCleaner | `test_data_cleaner.py` | 数据清洗器 | 10 |
| DataSplitter | `test_data_splitter.py` | 数据分割器 | 11 |
| FeatureCache | `test_feature_cache.py` | 缓存管理器 | 10 |

**总计**: 50+ 单元测试用例

### 集成测试

| 测试模块 | 测试文件 | 说明 |
|---------|---------|------|
| DataPipeline | `test_data_pipeline.py` | 完整流水线集成测试 |

## 🚀 快速开始

### 运行所有测试

```bash
cd core/tests
python3 run_all_tests.py
```

### 运行特定测试模块

```bash
# 运行 DataLoader 测试
python3 run_all_tests.py --module test_data_loader

# 运行 FeatureEngineer 测试
python3 run_all_tests.py --module test_feature_engineer

# 运行 DataCleaner 测试
python3 run_all_tests.py --module test_data_cleaner
```

### 运行单个测试文件

```bash
# 直接运行测试文件
python3 test_data_loader.py
python3 test_feature_engineer.py
python3 test_data_cleaner.py
python3 test_data_splitter.py
python3 test_feature_cache.py
```

### 调整详细程度

```bash
# 静默模式
python3 run_all_tests.py --verbosity 0

# 正常模式
python3 run_all_tests.py --verbosity 1

# 详细模式（默认）
python3 run_all_tests.py --verbosity 2
```

## 📊 测试内容

### test_data_loader.py

测试数据加载器的所有功能：

- ✅ 初始化和配置
- ✅ 成功加载数据
- ✅ 数据为空处理
- ✅ 数据为 None 处理
- ✅ 缺少必要列异常
- ✅ 日期索引转换
- ✅ 数据排序
- ✅ 数据库异常处理
- ✅ 无日期索引异常

### test_feature_engineer.py

测试特征工程器的所有功能：

- ✅ 初始化和配置
- ✅ 计算所有特征
- ✅ 技术指标计算（MA, EMA, RSI, MACD, KDJ, BOLL, ATR, OBV, CCI）
- ✅ Alpha因子计算（动量、波动率、成交量等）
- ✅ 特征转换（多时间尺度收益率、OHLC、时间特征）
- ✅ 特征去价格化
- ✅ 目标标签创建
- ✅ 特征数量验证
- ✅ NaN 处理
- ✅ 不同预测周期

### test_data_cleaner.py

测试数据清洗器的所有功能：

- ✅ 初始化和配置
- ✅ 清洗正常数据
- ✅ 处理无穷值
- ✅ 移除特征 NaN
- ✅ 移除目标 NaN
- ✅ 截断极端值
- ✅ 统计信息追踪
- ✅ 空数据处理
- ✅ 全NaN特征处理
- ✅ 数据保留率验证

### test_data_splitter.py

测试数据分割器的所有功能：

- ✅ 初始化和配置
- ✅ 时间序列分割
- ✅ 鲁棒缩放 (Robust Scaling)
- ✅ 标准缩放 (Standard Scaling)
- ✅ MinMax 缩放
- ✅ 欠采样平衡 (Undersample)
- ✅ 过采样平衡 (Oversample)
- ✅ Scaler 持久化
- ✅ 设置外部 Scaler
- ✅ 平衡数据跳过重采样
- ✅ 性能优化验证（O(n) 算法）

### test_feature_cache.py

测试缓存管理器的所有功能：

- ✅ 初始化和配置
- ✅ 生成缓存路径
- ✅ 保存和加载
- ✅ 缓存验证
- ✅ 版本不匹配处理
- ✅ 清除缓存
- ✅ 计算特征配置哈希
- ✅ 损坏缓存处理
- ✅ 缺少元数据处理
- ✅ 缓存元数据内容验证

## 🔧 测试环境

### 依赖项

测试依赖以下Python包（已在 requirements.txt 中）：

```
pandas
numpy
scikit-learn
imbalanced-learn
loguru
```

### Mock 测试

组件级测试使用 Mock 对象，无需真实数据库连接。这使得测试：

- ✅ 快速（秒级完成）
- ✅ 独立（不依赖外部资源）
- ✅ 可靠（不受数据变化影响）
- ✅ 隔离（只测试目标组件）

## 📈 测试报告示例

```
================================================================================
                            Core 项目测试套件
================================================================================

================================================================================
运行模块: test_data_loader
================================================================================
...........
----------------------------------------------------------------------
Ran 9 tests in 0.123s

OK

================================================================================
运行模块: test_feature_engineer
================================================================================
..........
----------------------------------------------------------------------
Ran 10 tests in 1.234s

OK

================================================================================
                              测试总结报告
================================================================================

模块级别测试结果:
模块名称                           测试数     成功     失败     错误     跳过
--------------------------------------------------------------------------------
✓ test_data_loader                    9        9        0        0        0
✓ test_feature_engineer              10       10        0        0        0
✓ test_data_cleaner                  10       10        0        0        0
✓ test_data_splitter                 11       11        0        0        0
✓ test_feature_cache                 10       10        0        0        0
--------------------------------------------------------------------------------
总计                                  50       50        0        0        0

================================================================================
测试总数: 50
成功: 50 (100.0%)
失败: 0
错误: 0
跳过: 0
总耗时: 2.34秒
================================================================================

✓ 所有测试通过!
```

## 🎯 测试原则

### 1. 独立性
每个测试用例都是独立的，不依赖其他测试的结果。

### 2. 可重复性
测试使用固定的随机种子（`np.random.seed(42)`），确保结果可重复。

### 3. 完整性
覆盖正常情况、边界情况和异常情况。

### 4. 清晰性
每个测试都有清晰的名称和文档说明其目的。

### 5. 快速性
使用 Mock 对象避免慢速操作（如数据库、网络请求）。

## 🔍 调试测试

### 查看详细输出

```bash
python3 run_all_tests.py --verbosity 2
```

### 运行单个测试方法

```bash
cd core/tests
python3 -m unittest test_data_loader.TestDataLoader.test_01_init
```

### 使用 pytest（可选）

```bash
pip install pytest pytest-cov

# 运行所有测试
pytest

# 查看覆盖率
pytest --cov=src --cov-report=html

# 运行特定测试
pytest test_data_loader.py::TestDataLoader::test_01_init
```

## 📝 添加新测试

### 1. 创建测试文件

```python
#!/usr/bin/env python3
"""
MyComponent 单元测试
"""

import sys
import unittest
from pathlib import Path

project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root / 'src'))

from my_module import MyComponent


class TestMyComponent(unittest.TestCase):
    """测试 MyComponent 类"""

    def setUp(self):
        """每个测试前的准备"""
        self.component = MyComponent()

    def test_01_basic_function(self):
        """测试1: 基础功能"""
        result = self.component.do_something()
        self.assertEqual(result, expected_value)


if __name__ == '__main__':
    unittest.main()
```

### 2. 添加到测试运行器

编辑 `run_all_tests.py`，在 `test_modules` 列表中添加新模块：

```python
test_modules = [
    'test_data_loader',
    'test_feature_engineer',
    # ... 其他测试 ...
    'test_my_component',  # 新增
]
```

## 🐛 已知问题

### 数据库连接测试

`test_data_pipeline.py` 中的集成测试需要真实的数据库连接。如果数据库不可用，这些测试会被跳过（skip）。

解决方法：
- 使用 Mock 数据库（推荐）
- 配置测试数据库
- 跳过集成测试，仅运行单元测试

## 📚 参考资源

- [Python unittest 文档](https://docs.python.org/3/library/unittest.html)
- [Mock 对象使用指南](https://docs.python.org/3/library/unittest.mock.html)
- [pytest 文档](https://docs.pytest.org/)

## 🤝 贡献

添加新测试时，请确保：

1. ✅ 测试命名清晰（`test_XX_description`）
2. ✅ 包含文档字符串说明测试目的
3. ✅ 使用 Mock 对象避免外部依赖
4. ✅ 验证正常情况和异常情况
5. ✅ 测试运行时间 < 1秒（单个测试）
6. ✅ 更新此 README 文档
