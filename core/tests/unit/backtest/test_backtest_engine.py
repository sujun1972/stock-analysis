...
class TestBacktestThreeLayer:
    ...
    def sample_prices(self):
        ...
        # 生成有趋势的价格数据
        prices = pd.DataFrame(
            np.random.rand(len(dates), len(stocks)) * 2 + 10,
            index=dates,
            columns=stocks
        )
        ...
        # 添加一些趋势
        for i, stock in enumerate(stocks):
            trend = np.linspace(0, i * 0.5, len(dates))
            prices[stock] = prices[stock] + trend
    ...
    def mock_entry(self):
        ...
        class MockEntry(EntryStrategy):
            ...
            def generate_entry_signals(self, candidate_stocks, data, date):
                # 等权重买入所有候选股票
                return {stock: 1.0 for stock in candidate_stocks}
    ...
    def mock_exit(self):
        """创建模拟退出策略"""
        from src.strategies.three_layer.base.exit_strategy import ExitStrategy
    ...
    def test_three_layer_backtest_basic(
        self, sample_prices, mock_selector, mock_entry, mock_exit
    ):
        """测试基本的三层架构回测"""
        engine = BacktestEngine(initial_capital=1000000)
    ...
    def test_three_layer_equity_curve(
        self, sample_prices, mock_selector, mock_entry, mock_exit
    ):
        """测试净值曲线生成"""
        engine = BacktestEngine(initial_capital=1000000)
        ...
        assert all(equity_curve > 0)  # 净值应该始终为正

    def test_three_layer_trades_recording(
        self, sample_prices, mock_selector, mock_entry, mock_exit
    ):
        """测试交易记录"""
        engine = BacktestEngine(initial_capital=1000000)
        ...
        if len(trades) > 0:
            required_cols = ['date', 'stock_code', 'direction', 'shares', 'price', 'amount']
            for col in required_cols:
                assert col in trades.columns

            # 验证交易方向
            assert all(trades['direction'].isin(['buy', 'sell']))

            # 验证交易金额计算
            assert all(
                abs(trades['amount'] - trades['shares'] * trades['price']) < 0.01
            )

    def test_three_layer_metrics_calculation(
        self, sample_prices, mock_selector, mock_entry, mock_exit
    ):
        """测试绩效指标计算"""
        engine = BacktestEngine(initial_capital=1000000)
        ...
        assert metrics['max_drawdown'] <= 0  # 最大回撤应该为负数或0
        assert metrics['n_trades'] >= 0

    def test_three_layer_rebalance_frequency(
        self, sample_prices, mock_selector, mock_entry, mock_exit
    ):
        """测试不同调仓频率"""
        engine = BacktestEngine(initial_capital=1000000)
        ...
        # 测试日频
        result_daily = engine.backtest_three_layer(
            selector=mock_selector,
            entry=mock_entry,
            exit_strategy=mock_exit,
            prices=sample_prices,
        ...
        assert result_daily.is_success()

        # 测试周频
        result_weekly = engine.backtest_three_layer(
            selector=mock_selector,
            entry=mock_entry,
            exit_strategy=mock_exit,
            prices=sample_prices,
        ...
        assert result_weekly.is_success()

        # 测试月频
        result_monthly = engine.backtest_three_layer(
            selector=mock_selector,
            entry=mock_entry,
            exit_strategy=mock_exit,
            prices=sample_prices,
        ...
        assert result_monthly.is_success()

    def test_three_layer_with_real_strategies(self, sample_prices):
        """测试使用真实策略的三层回测"""
        ...
        selector = MomentumSelector(params={'lookback_period': 20, 'top_n': 5})  # 修改为5，满足参数验证
    ...
    def test_three_layer_empty_date_range(
        self, sample_prices, mock_selector, mock_entry, mock_exit
    ):
        """测试空日期范围"""
        engine = BacktestEngine(initial_capital=1000000)
        ...
        result = engine.backtest_three_layer(
            ...
            exit_strategy=mock_exit,
            prices=sample_prices,
            start_date='2025-01-01',  # 超出价格数据范围
            end_date='2025-01-31',
            rebalance_freq='W'
        )
    ...
    def test_three_layer_custom_capital(
        self, sample_prices, mock_selector, mock_entry, mock_exit
    ):
        """测试自定义初始资金"""
        engine = BacktestEngine(initial_capital=1000000)
        ...
        result = engine.backtest_three_layer(
            ...
            end_date='2023-02-28',
            rebalance_freq='W',
            initial_capital=5000000  # 使用自定义资金
        )

        assert result.is_success()
        equity_curve = result.data['equity_curve']
        assert equity_curve.iloc[0] == pytest.approx(5000000, rel=0.01)

    def test_three_layer_slippage_and_commission(
        self, sample_prices, mock_selector, mock_entry, mock_exit
    ):
        """测试滑点和佣金影响"""
        # 无成本回测
        engine1 = BacktestEngine(initial_capital=1000000)
        result1 = engine1.backtest_three_layer(
            selector=mock_selector,
            entry=mock_entry,
            exit_strategy=mock_exit,
            prices=sample_prices,
        ...
        result2 = engine2.backtest_three_layer(
            selector=mock_selector,
            entry=mock_entry,
            exit_strategy=mock_exit,
            prices=sample_prices,
        ...
        # 有成本的收益应该低于无成本
        if len(result1.data['trades']) > 0:
            assert (
                result2.data['metrics']['total_return'] <=
                result1.data['metrics']['total_return']
            )

    def test_three_layer_position_management(
        self, sample_prices, mock_selector, mock_entry, mock_exit
    ):
        """测试持仓管理"""
        engine = BacktestEngine(initial_capital=1000000)
        ...
        # 验证持仓历史记录
        positions_history = result.data['positions']
        assert isinstance(positions_history, list)

        # 如果有交易，应该有持仓记录
        trades = result.data['trades']
        if len(trades) > 0:
            assert len(positions_history) > 0
